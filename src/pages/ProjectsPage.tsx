...
const ProjectsPage: React.FC = () => {
    ...
    // Filter projects
    // Filter projects (Multi-category support)
    const filteredProjects = activeCategory === "Tous"
        ? projects
        : projects.filter((p) => {
            const projectCategories = p.category ? p.category.split(',').map(c => c.trim()) : [];
            // Check if active category is in the project's category list OR if the string contains it (fallback)
            return projectCategories.includes(activeCategory) || p.category.includes(activeCategory);
        });

    return (
        <div className="bg-jawaWhite dark:bg-jawaBlack min-h-screen flex flex-col transition-colors duration-300">
            <Header />
            ...
            <main className="flex-grow pt-32 pb-24">
                <div className="container-wide">
                    <SectionHeader
                        ...
                        description="Explorez l'ensemble de nos projets. Du site vitrine à l'application complexe, nous donnons vie aux ambitions digitales."
                        align="left"
                    />

                    {/* Filters - EXACT match to Portfolio.tsx */}
                    ...
                    {/* Grid - EXACT match to Portfolio.tsx */}
                    {loading ? (
